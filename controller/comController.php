...
class comController extends Controller{
    ...
    public $initphp_list = array('getcommentdir', 'getcommentln', 'getcommentid', 'getcomments_json', 'addcomment_json', 'updatecomment_json', 'delcomment_json', 
	...
	public function getcommentid(){
		if(!checkrequestpara(array("id"))){
			echo sprintf(return_error_template_json, PARA_ERR, '"id shuld be defined"');
			return;	
		}
		$dat = InitPHP::getDao('com');
		$results = $dat->get_comments_by_fields(array("id"=>$_REQUEST["id"]));
		if( empty($results) ){
			echo sprintf(return_error_template_json, NO_RECORD, '"look for table failed"');
			return;
		}
		$ret_json = make_json($results[0]);
	...
	public function getlatestfcs(){
		...
		$results = $dat->get_all_counted("flowcharts", $num);
		if( empty($results) ){
			echo sprintf(return_error_template_json, NO_RECORD, '"look for table failed"');
			return;
		}
