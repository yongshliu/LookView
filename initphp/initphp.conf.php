 ...
 * 是否开启调???
 */
$InitPHP_conf['is_debug'] = true; //开???正式上线请关???
$InitPHP_conf['show_all_error'] = false; //是否显示所有错误信息，必须在is_debug开启的情况下才能显???
/**
 * 日志目录，非常重???
 * 记录全局的框架层面的异常ERROR
 * 记录使用logInit工具类报的异常错误日???
 */
$InitPHP_conf['log_dir'] = '/home/ysliu/logs/'; //日志目录,必须配置
/**
 * 路由访问方式
 * 1. 如果为true 则开启path访问方式，否则关???
...
/**
 * 是否开启输出自动过???
 * 1. 对多人合作，安全性可控比较差的项目建议开???
 * 2. 对HTML进行转义，可以放置XSS攻击
 * 3. 如果不开启，则提供InitPHP::output()函数来???滤
 */
...
//default数据库配???一般使用中 $this->init_db('default')-> 或???$this->init_db()-> 为默认的模型
$InitPHP_conf['db']['default']['db_type']                   = 0; //0-单个服务器，1-读写分离???-随机
$InitPHP_conf['db']['default'][0]['host']                   = 'localhost'; //主机
$InitPHP_conf['db']['default'][0]['username']               = 'root'; //数据库用户名
$InitPHP_conf['db']['default'][0]['password']               = '1234'; //数据库密???
$InitPHP_conf['db']['default'][0]['database']               = 'hdm131470224_db'; //数据???
$InitPHP_conf['db']['default'][0]['charset']                = 'utf8'; //数据库编???  
$InitPHP_conf['db']['default'][0]['pconnect']               = 0; //是否持久链接

/*
* this is worked for a Linux datebase
*/
$InitPHP_conf['db']['linux']['db_type']                   = 0; //0-单个服务器，1-读写分离???-随机
$InitPHP_conf['db']['linux'][0]['host']                   = 'localhost'; //主机
$InitPHP_conf['db']['linux'][0]['username']               = 'root'; //数据库用户名
$InitPHP_conf['db']['linux'][0]['password']               = '1234'; //数据库密???
$InitPHP_conf['db']['linux'][0]['database']               = 'hdm131470224_db'; //数据???
$InitPHP_conf['db']['linux'][0]['charset']                = 'utf8'; //数据库编???  
$InitPHP_conf['db']['linux'][0]['pconnect']               = 0; //是否持久链接

/*
* this is worked for a yun datebase
*/
$InitPHP_conf['db']['yun']['db_type']                   = 0; //0-单个服务器，1-读写分离???-随机
$InitPHP_conf['db']['yun'][0]['host']                   = '121.40.105.6'; //主机
$InitPHP_conf['db']['yun'][0]['username']               = 'hdm131470224'; //数据库用户名
$InitPHP_conf['db']['yun'][0]['password']               = 'Splendiddata'; //数据库密???
$InitPHP_conf['db']['yun'][0]['database']               = 'hdm131470224_db'; //数据???
$InitPHP_conf['db']['yun'][0]['charset']                = 'utf8'; //数据库编???  
$InitPHP_conf['db']['yun'][0]['pconnect']               = 0; //是否持久链接

//test数据库配???使用???this->init_db('test')->  支持读写分离，随机选择（有两个数据库）
$InitPHP_conf['db']['test']['db_type']                      = 2; //0-单个服务器，1-读写分离???-随机
$InitPHP_conf['db']['test'][0]['host']                      = '127.0.0.1'; //主机
$InitPHP_conf['db']['test'][0]['username']                  = 'root'; //数据库用户名
$InitPHP_conf['db']['test'][0]['password']                  = ''; //数据库密???
$InitPHP_conf['db']['test'][0]['database']                  = 't1'; //数据???
$InitPHP_conf['db']['test'][0]['charset']                   = 'utf8'; //数据库编???  
...
$InitPHP_conf['db']['test'][1]['password']                  = ''; //数据库密???
$InitPHP_conf['db']['test'][1]['database']                  = 't1'; //数据???
$InitPHP_conf['db']['test'][1]['charset']                   = 'utf8'; //数据库编???  
$InitPHP_conf['db']['test'][1]['pconnect']                  = 0; //是否持久链接

/*********************************Service配置*****************************************/
/**
 * Service配置参数
 * 1. 你可以配置service的路径和文件（类名称）的后缀???
 * 2. 一般情况下您不需要改动此配置
 */
...
$InitPHP_conf['controller']['controller_postfix']    = 'Controller'; //控制器文件后缀???
...
$InitPHP_conf['controller']['module_list']           = array('test', 'index'); //module白名???
...
$InitPHP_conf['template']['template_tag_left']  = '<!--{';//模板左标???
$InitPHP_conf['template']['template_tag_right'] = '}-->';//模板右标???
$InitPHP_conf['template']['is_compile']         = true;//模板每次编译-系统上线后可以关闭此功能
$InitPHP_conf['template']['driver']             = 'simple'; //不同的模板驱动编???
...
$InitPHP_conf['interceptor']['postfix'] = "Interceptor"; //拦截器类和文件的后缀???
$InitPHP_conf['interceptor']['rule'] = array( //拦截器规???
	'test' => array(
...
/**
 * RPC配置
 * RPC分两种，服务提供???provider和服务使用???customer
 */
...
/**
 * 插件Hook配置
 * 1. 如果你需要使用InitPHP::hook() 钩子函数来实现插件功???
 * 2. 详细查看钩子的使用方???
 */
...
/**
 * 缓存配置参数
 * 1. 您如果使用缓???需要配置memcache的服务器和文件缓存的缓存路径
 * 2. memcache可以配置分布式服务器，根???InitPHP_conf['memcache'][0]的KEY值去进行添加
 * 3. 根据您的实际情况配置
 */
$InitPHP_conf['memcache'][0]   = array('127.0.0.1', '11211');
$InitPHP_conf['cache']['filepath'] = 'data/filecache';   //文件缓存目录
/**
 * MongoDB配置，如果您使用了mongo，则需要配???
 */
