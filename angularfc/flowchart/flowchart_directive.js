...
.directive('flowChart', function() {
  return {
  	restrict: 'E',
  	templateUrl: "/angularfc/flowchart/flowchart_template.html",
  	replace: true,
  	scope: {
  		chart: "=chart",
  	},
	...
	$scope.nodeMouseDown = function (evt, node) {
		...
		dragging.startDrag(evt, {
			...
			dragging: function (x, y) {
				...
				// change the flowchart height to accomordate the notds
				var charViewHight = 0;
				var ht = node.y() + node.height();
				if( ht > charViewHight){
						charViewHight = ht;
				}
				var dht = $(".draggable-container").innerHeight();
				if( charViewHight && charViewHight > dht){
					chart.data.viewHeight = charViewHight; 
					$(".draggable-container").css("height", charViewHight);
				}
				var charViewWidth = $(".draggable-container").innerWidth();
				var wt = node.x() + node.width();
				if( wt > charViewWidth ){
					chart.data.viewWidth = wt;
					$(".draggable-container").css("width", wt);
				}
			},
