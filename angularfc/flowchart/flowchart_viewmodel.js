...
(function () {
	...
	flowchart.defaultNoeHeight = 200;

	flowchart.defaultLineChar = 15;
	//
	...
	flowchart.connectorSpace = 38; // the height + the space between each other

	//
	// Compute the Y coordinate of a connector, given its index.
	//
	flowchart.computeConnectorY = function (connectorIndex) {
		return /*flowchart.nodeNameHeight +*/ (connectorIndex * (flowchart.connectorSpace));
	}
	...
	flowchart.computeConnectorPos = function (node, connectorIndex, inputConnector) {
		return {
			x: node.x() + (connectorIndex == 0? node.width()/2 : node.width()),
			y: node.y() + (connectorIndex == 0? node.height():flowchart.computeConnectorY(connectorIndex)),
	...
	var createConnectorsViewModel = function (connectorDataModels, x, y, parentNode, input) {
		var viewModels = [];

		if (connectorDataModels) {
			for (var i = 0; i < connectorDataModels.length; ++i) {
				var connectorViewModel = 0;
				if( input ){
					if( i == 0 ){
						connectorViewModel = new flowchart.ConnectorViewModel(connectorDataModels[i], x/2, 0, parentNode);
					}else{
						connectorViewModel = new flowchart.ConnectorViewModel(connectorDataModels[i], 0, flowchart.connectorSpace*i, parentNode);
					}
				}else{
					if( i == 0 ){
						connectorViewModel = new flowchart.ConnectorViewModel(connectorDataModels[i], x/2, y, parentNode);
					}else{
						connectorViewModel = new flowchart.ConnectorViewModel(connectorDataModels[i], x, flowchart.connectorSpace*i, parentNode);
					}
				}
	...
	flowchart.NodeViewModel = function (nodeDataModel) {
		...
		this.namelist = [];
		this.alink=this.data.alink;
		var nodename = this.data.name || "no name";
		var noaline = flowchart.defaultLineChar;
		var nn = nodename.substr(0);
		nn = util_unescape(nn);
	//	nn = $(nn).html();
		nn = $("<p>"+nn+"</p>").text();
		//console.log($("<div>yes<p>hello</p></div>").text());
		var namelen = nn.length;
		for(var i =0;i<namelen;i+=noaline){
			var nameline = nn.slice(i, i+noaline<namelen?i+noaline:namelen);
			this.namelist.push(nameline);
		}
		// set the default width value of the node
		if (!this.data.width || this.data.width < flowchart.defaultNodeWidth) {
			this.data.width = flowchart.defaultNodeWidth;
		}
		
		
		this.data.height = (this.namelist.length*1.2+2)*15;//flowchart.defaultNoeHeight;
		var numConnectors = Math.max( this.data.inputConnectors.length, this.data.outputConnectors.length);
		this.data.height = Math.max(this.data.height, numConnectors*(flowchart.connectorSpace));
	
		this.inputConnectors = createConnectorsViewModel(this.data.inputConnectors, this.data.width, this.data.height, this, true);
		this.outputConnectors = createConnectorsViewModel(this.data.outputConnectors, this.data.width, this.data.height, this, false);
		...
		this.height = function () {
			...
			return Math.max(this.data.height, numConnectors*(flowchart.connectorSpace));
		...
		//
		this.getInputConnectorNum = function(){
			if(this.data.inputConnectors)
				return this.data.inputConnectors.length;
			return 0;
		}
		this.getOutputConnectorNum = function(){
			if(this.data.outputConnectors)
				return this.data.outputConnectors.length;
			return 0;
